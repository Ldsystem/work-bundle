...
2. Do not optimize task or phase cardinality. Assign every authoritative production path to one production owner; reject helper-only allocation while its production path is unowned. Responsibility ownership does not set task granularity: one owner may receive several dependency-ordered execution packets, including ordered reuse of a production file. Start with the accepted outcome and a capable local oracle. Identify candidate increments from evidenced source neighborhoods, dependencies, validation loops, review boundaries, and repair frontiers. Ask whether one executor can understand, implement, validate, review, and repair each complete packet without repeatedly expanding unrelated context. Choose one coherent task when an intermediate result would be unstable or unreviewable; choose ordered tasks when each predecessor yields a usable accepted intermediate result; choose contract-decoupled parallel tasks only across a sufficiently stable existing or task-produced boundary. A dependency edge or common owner alone proves neither a split nor a merge. Split independently owned entry points only when current repository evidence proves distinct production or repair seams. Keep a coherent mechanical increment with one production owner, oracle, and repair frontier together. Weigh expected total orchestration cost across execution, context, coordination, validation, review, and repair, alongside demonstrated reuse, feasibility, and stability. Line, token, source-ID, target-file, and task counts are diagnostic clues, not qualification thresholds. Do not create speculative splits unsupported by current authority or evidence.
...
4. Carry execution-workspace isolation, hydration, and cleanup policy into task and executor context; mutating siblings on the same execution path isolate via prepare_worktree or serialize even when write scopes are disjoint. Overlapping write paths require a strict dependency order and the predecessor's canonical accepted result before successor execution; unordered overlap cannot run concurrently.
5. At a genuine stable parallel seam, use the existing common contract or a task-produced common contract group. Contract-decoupled participants depend on that boundary and accepted prior handoffs, not sibling in-progress implementation output. Consider local contract tests for each side when they give capable oracles. Create a phase only for an actual barrier or convergence boundary, with explicit barrier ID, readiness evidence, and convergence owner; place joint validation after convergence. Work sequentially while the interface is unsettled. A stable contract permits parallel work but does not require it.
...
8. When execution proves a task materially under-decomposed, return to the plan and reslice only the affected unaccepted authority region around the newly evidenced seam. Preserve the original binding, baseline, and accepted unaffected regions; do not repeatedly enlarge the task.
...
For a candidate packet boundary, ask whether a likely change or failure stays within that packet and whether its result is verifiable without unfinished sibling work. Consider separating independently changing policy and adapters only when current evidence shows that change axis. If an interface is consequentially uncertain, a small walking vertical slice with an observable result and capable local test may settle it at reasonable cost; otherwise use the existing oracle and keep the coherent work together. Choose a design pattern only when it contains a demonstrated change or failure boundary. These are optional controller judgment prompts expressed through existing source IDs, interfaces, dependencies, validation, and task briefs, not a pattern checklist, score, compulsory contract artifact, or new qualification gate.
...
- responsibility ownership versus coherent single, ordered, or contract-decoupled parallel execution packets;
...
- every authoritative production path has one production owner, while each task is a coherent bounded execution packet with exact files/symbols, dependencies, steps, methodology, rules/skills, capable oracle, and measurable completion criteria;
