...
An accepted review is product authority, not terminal orchestration. The controller decides which knowledge, repository, CodeGraph, workspace, and commit actions apply and whether their evidence is sufficient. After performing or delegating them, pass their explicit `completed|not_applicable|blocked` dispositions and exact clean source baselines to `finalize-accepted-plan`. That helper records supplied facts, verifies baselines, and owns only canonical archive/index/binding-release mechanics. A blocked disposition keeps the flow finalization-required without changing product acceptance. Run `require-terminal-finalization` before claiming the orchestration is complete.
