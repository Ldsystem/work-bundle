...
An accepted review is product authority, not terminal orchestration. The controller decides which knowledge, repository, CodeGraph, workspace, and commit actions apply and whether their evidence is sufficient. After applicable work resolves, pass exact clean source baselines to `finalize-accepted-plan`; it owns only baseline verification and canonical archive/index/binding-release mechanics. If applicable work cannot finish, do not call the helper—the accepted flow remains finalization-required. Run `require-terminal-finalization` before claiming completion.
