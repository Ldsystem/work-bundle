...
description: 'Create implementation plans from project knowledge for features, refactors, upgrades, design, architecture, data, process, or infrastructure work.'
---

# Create Implementation Plan

Create an executable implementation plan for `${input:PlanPurpose}`.
...
- implementation instructions, validation, completion criteria.
- no copied spec sections beyond short one-line summaries.
...
Confirm required front matter and sections exist, paths and dependencies are explicit, execution context is carried forward through spec-ID references plus concrete file-level instructions, no executor is required to read `.work-bundle/knowledge/`, blockers become leading clarification/spec-repair tasks, indexes are updated, and every plan/phase/task has mandatory `executor-result` handoff criteria.
