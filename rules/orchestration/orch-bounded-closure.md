...
- Treat an accepted outcome as `finalization_required`, never terminal. The controller completes the applicable final audit actions, then calls `finalize-accepted-plan`; the helper verifies the clean exact source baselines and performs only archive, index, and binding-release mechanics. `require-terminal-finalization` admits the orchestration-complete claim only after that transaction is closed.
...
- Do not let a helper infer whether a final-audit action applies or whether its evidence is semantically sufficient. If the controller cannot complete an applicable action, leave the accepted flow finalization-required without changing its product verdict.
