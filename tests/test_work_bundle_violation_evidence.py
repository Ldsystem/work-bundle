...
CATALOG = (REPO_ROOT / "references" / "wb-violation-evidence.yaml").read_text(encoding="utf-8")


def prepare_cwd(tmp_path: Path, catalog: str = CATALOG) -> Path:
    cwd = tmp_path / "cwd"
    (cwd / "references").mkdir(parents=True, exist_ok=True)
    (cwd / "references" / "wb-violation-evidence.yaml").write_text(catalog, encoding="utf-8")
    return cwd
...
def test_violation_ensure_store_creates_directories(tmp_path: Path) -> None:
    result = run_wb(tmp_path, "violation-ensure-store")
    ...
    assert Path(payload["root"]) == tmp_path / "config" / "violation"
...
def test_violation_create_evidence_rejects_invalid_severity(tmp_path: Path) -> None:
    result = run_wb(
        tmp_path,
        "violation-create-evidence",
        "--status",
...
def test_violation_create_evidence_rejects_archived_without_action(tmp_path: Path) -> None:
    result = run_wb(
        tmp_path,
        "violation-create-evidence",
        "--status",
...
def test_violation_create_evidence_rejects_status_directory_mismatch(tmp_path: Path) -> None:
    create_active(tmp_path, "status-mismatch")
    path = tmp_path / "config" / "violation" / "active" / f"{evidence_id('status-mismatch')}.yaml"
    path.write_text(path.read_text(encoding="utf-8").replace("status: active", "status: archived"), encoding="utf-8")

    result = run_wb(tmp_path, "violation-build-index")
...
def test_violation_build_index_outputs_active_and_archived_maps(tmp_path: Path) -> None:
    ...
    archived = run_wb(
        ...
        "archived-contract",
        "--deviation",
        "Archived work-bundle violation.",
        "--occurrence",
        "Created as archived.",
        "--evidence",
        "visible",
    ...
    assert 'deviation: "Archived work-bundle violation."' in result.stdout
    assert "action: dismiss" in result.stdout


def test_violation_write_index_writes_deterministic_index(tmp_path: Path) -> None:
    create_active(tmp_path, "write-index")
    built = run_wb(tmp_path, "violation-build-index")
    assert built.returncode == 0, built.stdout + built.stderr

    result = run_wb(tmp_path, "violation-write-index")
...
def test_violation_archive_evidence_moves_record(tmp_path: Path) -> None:
    created = create_active(tmp_path, "archive-me")

    result = run_wb(tmp_path, "violation-archive-evidence", str(created["id"]), "--action", "completed")
...
def test_violation_duplicate_identical_create_reports_changed_false(tmp_path: Path) -> None:
...
def test_violation_conflicting_duplicate_fails_without_overwrite(tmp_path: Path) -> None:
...
def test_violation_dispatcher_routes_all_commands_and_command_help(tmp_path: Path) -> None:
    for command in [
